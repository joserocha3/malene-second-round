...
<template>
  <div class="relative p-12 rounded-lg bg-neutral-100 shadow">
    ...
    <div class="absolute top-4 right-4 flex gap-2">
      <button class="duration-500 transform hover:-translate-y-1" @click="share({ title: props.album.title.label, text: 'Hey friend 👋 Check out this top album on itunes!', url: props.album.link.attributes.href })">
      ...
      <div class="cursor-pointer duration-500 transform hover:-translate-y-1">
        <input :id="props.album.title.label" class="hidden peer w-4 h-4 text-red-600 bg-gray-100 border-gray-300 rounded focus:ring-red-500 dark:focus:ring-red-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" type="checkbox" v-model="props.album.favorite">
        <label :for="props.album.title.label">
          <svg v-if="props.album.favorite" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#e22e26" class="w-6 h-6">
            <path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z" />
          </svg>
          <svg v-if="!props.album.favorite" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#e22e26" class="w-6 h-6">
